...
void misc::client::logger::events(i_game_event* event) {
	...
	if (get_name == utilities::fnv::hash("player_hurt")) {
		...
		notification_system::notify((std::stringstream{ } << "hit " << name.c_str() << " in the " << hit_group << " for " << damage << " damage (" << health << " remaining)").str().c_str(), color::white(variables::logger_clr[3] * 255.f));
	}

	if (get_name == utilities::fnv::hash("item_purchase")) {
		if (!variables::log_buys)
			return;
		...
		notification_system::notify((std::stringstream{ } << name.c_str() << " bought " << weapon).str().c_str(), color::white(variables::logger_clr[3] * 255.f));
	}
}
// change spacing from name - health etc.
void misc::client::notification_system::draw() {
	...
	for (auto i = 0; i < notify_list.size(); i++) {
		...
		//render::text(3, 10 + (i * 15), render::fonts::watermark_font, ("[customhooks]"), false, color(variables::logger_clr[0] * 255.f, variables::logger_clr[1] * 255.f, variables::logger_clr[2] * 255.f, variables::logger_clr[3] * 255.f));
		render::text(5, 5 + (i * 15), render::fonts::watermark_font, notify.text.c_str(), false, notify._color);
	}
}

void misc::client::notification_system::notify(std::string text, color _color) {
	if (text.empty())
		return;

	notify_list.push_back(notify_t(text, _color));
	// i dont know what im doing so im quitting on this
	interfaces::console->console_color_printf({ static_cast<unsigned char>(variables::logger_clr[0] * 255.f),
		static_cast<unsigned char>(variables::logger_clr[1] * 255.f),
		static_cast<unsigned char>(variables::logger_clr[2] * 255.f), 255 }, ("[customhooks]"));
