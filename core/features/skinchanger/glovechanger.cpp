...
static auto make_glove(int entry, int serial) -> attributable_item_t* 
{
	...
	assert(glove); {
		static auto set_abs_origin_addr = utilities::scan_pattern(GetModuleHandleA(xorstr_("client.dll")), xorstr_("55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8"));
...
bool skins::glovechanger::apply_glove_model(attributable_item_t* glove, const char* model)
{
	player_info_t info;
...
bool skins::glovechanger::apply_glove_skin(attributable_item_t* glove, int item_definition_index, int paint_kit, int model_index, int entity_quality, float fallback_wear)
{
...
void skins::glovechanger::run()
{
	...
	if (variables::glove_model > 0) {
		...
		if (glove)
		{
			...
			glove->fallback_wear() = variables::glove_wear;
